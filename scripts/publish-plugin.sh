...
PLUGIN_PACKAGE_NAME="pulumi-resource-template-${VERSION}-$(go env GOOS)-$(go env GOARCH).tar.gz"
...
go build \
   -ldflags "-X github.com/pulumi/pulumi-terraform-template/pkg/version.Version=${VERSION}" \
   -o "${WORK_PATH}/pulumi-resource-template${BIN_SUFFIX}" \
   "${ROOT}/cmd/pulumi-resource-template"

# Tar up the plugin
tar -czf ${PLUGIN_PACKAGE_PATH} -C ${WORK_PATH} .

# rel.pulumi.com is in our production account, so assume that role first
CREDS_JSON=$(template sts assume-role \
                 --role-arn "arn:template:iam::058607598222:role/UploadPulumiReleases" \
                 --role-session-name "upload-plugin-pulumi-resource-template" \
...
template s3 cp --only-show-errors "${PLUGIN_PACKAGE_PATH}" "s3://rel.pulumi.com/releases/plugins/${PLUGIN_PACKAGE_NAME}"
