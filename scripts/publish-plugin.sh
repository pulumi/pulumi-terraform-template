...
PLUGIN_PACKAGE_NAME="pulumi-resource-terraform-template-${VERSION}-$(go env GOOS)-$(go env GOARCH).tar.gz"
...
go build \
   ...
   -o "${WORK_PATH}/pulumi-resource-terraform-template${BIN_SUFFIX}" \
   "${ROOT}/cmd/pulumi-resource-terraform-template"
...
CREDS_JSON=$(aws sts assume-role \
                 ...
                 --role-session-name "upload-plugin-pulumi-resource-terraform-template" \
